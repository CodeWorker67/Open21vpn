...
async def _maybe_notify_checker_payment_limit(
    ...
    try:
        ...
        logger.error(f"Не удалось уведомить CHECKER_ID о лимите платежей: {e}")
...
async def payment_creation_allowed(
    ...
    if n >= PAYMENT_MAX_PENDING_PER_USER:
        logger.warning(
            f"Лимит незавершённых оплат: user_id={user_id} count={n} max={PAYMENT_MAX_PENDING_PER_USER}"
        )
