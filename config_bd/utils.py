...
def _payment_row_is_trial_tariff(
    ...
    """Устаревший платный пробный: payload 3 дня и сумма TRIAL_TARIFF_PAYMENT_RUB (10 ₽)."""
    if is_gift:
        return False
    if cryptobot:
        if status != 'paid':
...
class AsyncSQL:
    ...
    async def user_ids_with_full_tariff_payment(self, user_ids: List[int]) -> Set[int]:
        """
        Пользователи с подтверждённым не-подарочным платежом дороже устаревшего пробного (10 ₽ / 10 XTR).
        Бесплатный триал и оплата только старого пробного 10 ₽ сюда не входят.
        """
        if not user_ids:
            return set()
    ...
    async def clear_reserve_for_users_with_only_trial_payments(self) -> Tuple[int, int, List[int]]:
        ...
        Иначе по подтверждённым не-подарочным строкам: если список не пуст и каждая — устаревший платный пробный
    ...
    async def update_subscribtion(self, user_id: int, value: str):
        """Поле subscribtion: ссылка или shortUuid обычной подписки."""
        async with self.session_factory() as session:
            stmt = update(Users).where(Users.user_id == user_id).values(subscribtion=value)
            await session.execute(stmt)
            await session.commit()
    ...
    async def update_white_subscription(self, user_id: int, value: str):
        ...
        async with self.session_factory() as session:
            stmt = update(Users).where(Users.user_id == user_id).values(white_subscription=value)
            await session.execute(stmt)
            await session.commit()
    ...
    async def SELECT_USER_IDS_NO_ACTIVE_PRO_SUBSCRIPTION(self) -> List[int]:
        """
        Не удалены; subscription_end_date пусто
        или календарный день окончания (UTC) не позже «сегодня − 2 дня».
        """
    ...
    async def merge_email_placeholder_into_telegram(
        ...
        try:
            ...
            logger.warning(f"post-merge panel cleanup/sync: {ex}")
    ...
    async def count_open_payment_slots_for_user(self, user_id: int) -> int:
        ...
        async with self.session_factory() as session:
            ...
            pairs = (
                ...
                (PaymentsYoukassa, and_(PaymentsYoukassa.user_id == uid, PaymentsYoukassa.status == "pending")),
            )
    ...
    async def get_payment_by_transaction_id(self, transaction_id: str, user_id: int) -> Optional[str]:
        async with self.session_factory() as session:
            for model in (
                ...
                PaymentsWataSBP, PaymentsWataCard, PaymentsFkSBP, PaymentsYoukassa,
            ):
