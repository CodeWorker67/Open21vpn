"""
HTTP API для сайта Open 21 VPN и страницы подписки.
...
def _send_smtp_reset_email(to_email: str, code: str) -> None:
    ...
    msg["Subject"] = "Сброс пароля — Open 21 VPN"
    msg["From"] = SMTP_FROM
    msg["To"] = to_email
...
async def _deliver_reset_code(email: str, code: str, row: tuple) -> None:
    ...
    if SMTP_HOST and SMTP_FROM:
        try:
            ...
            logger.warning(f"SMTP password reset failed: {e}")
    if not smtp_ok and tg is not None:
        try:
            await bot.send_message(tg, f"Код сброса пароля: {code}")
        except Exception as e:
            logger.warning(f"Telegram password reset failed: {e}")
    if not smtp_ok and tg is None:
        logger.warning(f"Password reset code for {email} not delivered (configure SMTP or Telegram)")
...
async def _bot_deeplink_for_sub_page() -> str:
    ...
    try:
        ...
        logger.warning(f"sub_page pay: bot.get_me failed: {e}")
    return "https://t.me/"
...
def _send_smtp_verification_email(to_email: str, code: str) -> None:
    ...
    msg = MIMEText(body, "plain", "utf-8")
    msg["Subject"] = "Подтверждение email — Open 21 VPN"
    msg["From"] = SMTP_FROM
    msg["To"] = to_email
    with smtplib.SMTP(SMTP_HOST, SMTP_PORT, timeout=30) as s:
        if SMTP_USER and SMTP_PASSWORD:
...
async def _send_verification_code(email: str) -> str:
    ...
    try:
        ...
        logger.warning(f"SMTP verification email failed: {e}")
    return code
